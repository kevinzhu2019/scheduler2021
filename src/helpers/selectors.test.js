...
const state = {
  ...
  appointments: {
    ...
    "4": { id: 4, time: "2pm", interview: null },
    "5": {
      id: 5,
      time: "3pm",
    ...
    "6": {
      id: 6,
      time: "4pm",
      interview: { student: "shabi", interviewer: 4 }
    }
  },
  interviewers: {
    "1": {  
...
test("getInterviewersForDay returns an array containing the correct appointment objects", () => {
  ...
  expect(fourth).toEqual(state.interviewers["4"]);
});
...
test("getDailySpots should return a number", () => {
  const result = getSpotsForDay(state, "Monday");
  expect(typeof(result)).toEqual("number");
})

test("getDailySpots for Monday should equal to 3", () => {
  const result = getSpotsForDay(state, "Monday");
